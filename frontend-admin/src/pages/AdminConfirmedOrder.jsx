  ...
  useEffect(() => {
    ...
    const fetchAllCounts = async () => {
      try {
        ...
        setOrders(confirmedRes.data.orders);
        setFilteredOrders(confirmedRes.data.orders);
        setCounts({
  ...
  if (loading) {
    return (
      <div className="min-h-screen p-6 bg-gray-50 flex justify-center items-center">
        <p className="text-gray-700">Loading confirmed orders...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-50">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Confirmed Orders</h1>
        ...
        {filteredOrders.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            <p className="text-gray-500">
              {searchTerm ? 'No matches for your search' : 'There are currently no confirmed orders'}
            </p>
          </div>
        ) : (
