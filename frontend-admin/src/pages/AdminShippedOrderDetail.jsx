  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        ...
        const response = await axiosInstance.get(`/api/order-tracking/admin/shipped-orders/${orderId}`, {
          headers: {
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      ...
      await axiosInstance.put(
        `/api/order-tracking/admin/shipped-orders/${orderId}/status`,
        { status: newStatus },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setStatusUpdate(newStatus); // Triggers re-fetch
    } catch (error) {
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-t pt-4">
            ...
            <div className="flex space-x-3 w-full md:w-auto">
              {order.status === 'shipped' && (
                <button
                  onClick={() => navigate(`/admin/orders/${order._id}/assign-rider`)}
...
export default AdminShippedOrderDetail;