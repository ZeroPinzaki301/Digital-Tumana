...
const CartOrderPreviewPage = () => {
  ...
  useEffect(() => {
    if (location.state && location.state.selectedItemIds) {
      setSelectedItemIds(location.state.selectedItemIds);
    }
  }, [location.state]);
  ...
  useEffect(() => {
    const fetchPreview = async () => {
      try {
        ...
        const res = await axiosInstance.post("/api/carts/preview", 
        ...
        console.log("DeliveryTo data:", res.data.deliveryTo);
        ...
        setAlertMessage("Failed to load cart preview. Please try again.");
              ...
              <div className="flex justify-end">
                <button
                  onClick={() => {
                    ...
                    if (successMessage === "Orders placed successfully!") {
                      navigate("/customer/ongoing-orders");
                    }
                  }}
                  className="px-4 py-2 bg-lime-700 text-white rounded hover:bg-lime-600/75 cursor-pointer"
