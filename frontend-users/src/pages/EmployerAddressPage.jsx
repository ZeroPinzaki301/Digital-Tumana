...
const EmployerAddressPage = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Fetch address data
        try {
          const addressRes = await axiosInstance.get("/api/employers/address", {
            headers: { Authorization: `Bearer ${token}` },
          });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        const checkRes = await axiosInstance.get("/api/employers/address", {
          headers: { Authorization: `Bearer ${token}` },
        });
      ...
      const res = await axiosInstance[method]("/api/employers/address", formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="min-h-screen bg-emerald-100 flex items-center justify-center px-4 relative">
      ...
      <div className="bg-white w-full max-w-2xl rounded-lg p-6 shadow-md border border-lime-700">
        <h2 className="text-2xl font-bold text-lime-700 mb-6 text-center">Employer Address</h2>
