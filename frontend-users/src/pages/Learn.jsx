...
const Calendar18Icon = ({ className = 'w-7 h-7' }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none">
    ...
    <text x="12" y="16.5" textAnchor="middle" fontSize="6" fontFamily="ui-sans-serif" fill="currentColor">18+</text>
  </svg>
);

// SVG Background for TESDA section
const tesdaBgSvg = `
...
const Learn = () => {
  ...
  return (
    <div className="text-white">
      {/* HERO */}
      <section
        className="relative bg-cover bg-center bg-no-repeat h-[580px] md:h-[640px] px-4 sm:px-6 md:px-10 py-10 md:py-20"
      ...
      >
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between h-full max-w-7xl mx-auto">
          <div className="w-full md:w-1/2 flex justify-center md:justify-start">
            <img src={tesdaLogo} alt="TESDA logo" className="h-[300px] sm:h-[380px] md:h-[500px] object-contain drop-shadow-[0_0_30px_rgba(255,255,255,0.7)]" />
          </div>

          <div className="w-full md:w-1/2 flex flex-col items-center md:items-end text-center md:text-right mt-8 md:mt-0">
            ...
            <h2 className="mt-4 text-3xl sm:text-4xl md:text-6xl leading-tight font-extrabold">
            ...
            <div className="mt-6 flex flex-wrap gap-3 justify-center md:justify-end">
              <button onClick={() => document.getElementById('requirements')?.scrollIntoView({ behavior: 'smooth' })}
                className="rounded-xl bg-emerald-200/90 text-black font-semibold px-5 py-3 hover:bg-lime-300 transition shadow cursor-pointer">
                View Requirements
              </button>
              <button onClick={() => navigate('/learn/tesda/enroll')}
      ...
      {/* Service Cards */}
      <section className="w-full bg-white px-4 sm:px-6 md:px-10 py-10 space-y-8">
        {[{ img: plantingImage, text: "TURN YOUR PLANTING HOBBIES TO THE NEXT LEVEL", reverse: true },
        { img: methodsImage, text: "GET TO LEARN NEW METHODS AND TECHNIQUES", reverse: false },
        { img: collabImage, text: "AND COLLABORATE WITH FELLOW KA-TUMANA!", reverse: true }]
          .map(({ img, text, reverse }, i) => (
            <div key={i} className={`group w-full flex flex-col ${reverse ? "md:flex-row-reverse" : "md:flex-row"} h-auto md:h-[500px] overflow-hidden rounded-lg`}>
              <div className="relative w-full md:w-[65%] h-[250px] md:h-auto bg-cover bg-center" style={{ backgroundImage: `url(${img})` }}>
                <div className="absolute inset-0 bg-black/50 transition group-hover:bg-lime-600/20" />
                <div className={`absolute top-0 ${reverse ? "left-0" : "right-0"} h-full w-1/3 bg-gradient-to-${reverse ? 'l' : 'r'} from-transparent to-gray-50`} />
              </div>
              <div className="w-full md:w-[35%] bg-white flex items-center justify-center px-6 text-center py-10 md:py-0">
                <p className="text-xl sm:text-2xl md:text-4xl font-semibold text-black">{text}</p>
              </div>
            </div>
          ))}
      </section>
      ...
      {/* Requirements */}
      <section id="requirements" className="relative text-black py-16 px-4 sm:px-6 md:px-10 overflow-hidden">
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url('${tesdaBgUrl}')` }} />
        <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px]" />
        <div className="relative z-10 max-w-7xl mx-auto">
          <div className="text-center max-w-3xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-extrabold">TESDA Enrollment Requirements</h2>
            <p className="mt-2 text-neutral-700">Bring originals and one (1) photocopy for verification.</p>
          </div>

          <div className="mt-10 grid gap-5 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
            {[{
              icon: <Calendar18Icon />, title: "At least 18 years old", desc: "Proof of age may be requested during enrollment."
            }, {
              icon: <IdCardIcon />, title: "Valid ID (any government‑issued)", desc: "e.g., PhilID, Driver’s License, Passport, UMID, etc."
            }, {
              icon: <CertificateIcon />, title: "Birth Certificate (photocopy)", desc: "For identity verification and record matching."
            }].map(({ icon, title, desc }, i) => (
              <div key={i} className="relative group rounded-2xl bg-white/80 border border-lime-200 shadow hover:shadow-md transition p-5">
                <div className="flex items-start gap-3">
                  <div className="p-2 rounded-xl bg-lime-100 text-lime-700">{icon}</div>
                  <div>
          ...
          <div className="mt-8 mx-auto max-w-3xl rounded-2xl bg-white/80 border border-neutral-200 p-5">
            ...
            <button onClick={() => navigate('/learn/tesda/enroll')}
              className="mt-5 rounded-xl bg-[#91CA4C] text-black font-semibold px-4 py-2 text-sm hover:bg-lime-600 shadow cursor-pointer">
              Enroll Now
            </button>
          </div>
        </div>
      ...
      {/* Kariton Riders */}
      <section className="relative bg-gradient-to-r from-[#91CA4C] to-[#4B8B3B] text-white py-20 px-4 sm:px-6 md:px-10">
        <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/green-dust-and-scratches.png')] opacity-20" />
        <div className="relative z-10 max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-10">
          <div className="flex-1 text-center md:text-left">
            <h2 className="text-3xl sm:text-4xl md:text-6xl font-bold mb-6 drop-shadow-lg">Become a Kariton Rider</h2>
            <p className="text-lg sm:text-xl md:text-2xl mb-6 max-w-xl mx-auto md:mx-0">
            ...
            <div className="flex items-start justify-center md:justify-start text-left text-lg md:text-xl mb-8 gap-3">
              <img src="https://cdn-icons-png.flaticon.com/512/684/684908.png" alt="Location Icon" className="w-10 h-10" />
              <div>
                <p><b>Application Site:</b> Angel Tolits Integrated Farm</p>
                <p><b>Address:</b> #43 Donacion, Angat, Bulacan</p>
              </div>
            </div>
          </div>

          <div className="flex-1 flex justify-center md:justify-end gap-6 flex-wrap">
            <img src={karitonMotorcycle} alt="Motorcycle Rider" className="w-24 h-24 md:w-36 md:h-36 object-contain drop-shadow-xl" />
            <img src={karitonTricycle} alt="Tricycle" className="w-24 h-24 md:w-36 md:h-36 object-contain drop-shadow-xl" />
            <img src={karitonCar} alt="Car" className="w-24 h-24 md:w-36 md:h-36 object-contain drop-shadow-xl" />
          </div>
        </div>
      </section>
    </div>
